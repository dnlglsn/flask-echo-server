...
def extract(d):
    return {key.lower(): value.lower() for (key, value) in d.items()}
...
def echo(path):
    ...
    response.headers = {'sm-feature-set-ids': 'set from origin server'}

    if config[WAIT] > 0:
        print('Waiting for %i seconds' % config[WAIT])
...
def main():
    ...
    parser.add_option('--wait', dest='wait', default=0, help='how long to wait before sending a response', type=int)
